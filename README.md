...
5. Waits 24 hours and repeats the check.
...
- `update_csv()` exists in `main.py`, but the main loop does not currently call it. The baseline is therefore not refreshed automatically, and the same unfollow may be reported again on subsequent checks.
